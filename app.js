...
function showInviteBanner(inv) {
  ...
  el.innerHTML =
    `<div class="ib-text">🎮 <b>${esc(inv.from_name || 'Jemand')}</b> lädt dich zu Zauberstich ein</div>` +
...
function notifyYourTurn() {
  ...
  try {
    const n = new Notification('Zauberstich – du bist dran! 🧙', { body: 'Tippe, um weiterzuspielen.', tag: 'wiz-turn', icon: './icon-192.png' });
