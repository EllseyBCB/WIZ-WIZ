...
export function allCardImageUrls() {
  ...
  for (const c of ['R', 'Y', 'G', 'B']) for (let r = 1; r <= 13; r++) urls.push(`${base}/${c}${r}.png?v=16`);
  for (let i = 1; i <= 4; i++) { urls.push(`${base}/Z${i}.png?v=16`); urls.push(`${base}/N${i}.png?v=16`); }
  return urls;
}
...
export function preloadCards() {
  ...
  codes.forEach((code, i) => {
    setTimeout(() => { const im = new Image(); im.decoding = 'async'; im.src = `${base}/${code}.png?v=16`; }, i * 40);
  });
}
...
export function renderCard(code, opts = {}) {
  ...
  if (CARD_IMAGE_BASE) {
    ...
    img.decoding = 'async';
    img.src = `${deckBase()}/${code}.png?v=16`;
